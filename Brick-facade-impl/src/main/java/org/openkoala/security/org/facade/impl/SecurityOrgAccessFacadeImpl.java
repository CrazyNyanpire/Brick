...
public class SecurityOrgAccessFacadeImpl implements SecurityOrgAccessFacade {
	...
	@Override
	public Page<EmployeeUserDTO> pagingQueryEmployeeUsers(int pageIndex,
			int pageSize, EmployeeUserDTO queryEmployeeUserCondition) {
		Map<String, Object> conditionVals = new HashMap<String, Object>();
		StringBuilder jpql = new StringBuilder(
				"FROM EmployeeUser _user where 1=1");
		assembleUserJpqlAndConditionValues(queryEmployeeUserCondition, jpql,
				"_user", conditionVals);

		Page<EmployeeUser> results = getQueryChannelService()
				.createJpqlQuery(jpql.toString())//
				.setParameters(conditionVals)//
				.setPage(pageIndex, pageSize)//
				.pagedList();

		return new Page<EmployeeUserDTO>(results.getStart(),
				results.getResultCount(), pageSize,
				transformToEmployeeUserDTO(results.getData()));
	}

	private List<EmployeeUserDTO> transformToEmployeeUserDTO(
		...
		for (EmployeeUser employeeUser : employeeUsers) {
			EmployeeUserDTO result = EmployeeUserAssembler
					.toEmployeeUserDTO(employeeUser);
			...
			if (employeeUser.getEmployee() != null) {
				...
				Organization organization = employeeUser.getEmployee()
						.getOrganization(new Date());
	...
	private List<OrgRoleDTO> transformToOrgRoleDTO(
		...
		for (Authorization authorization : authorizations) {
			...
			if (authorization.getAuthority() instanceof Role) {
				...
				result = new OrgRoleDTO(role.getId(), role.getName(),
						role.getDescription());
			}
			// 直接使用范围的名称，因为范围保持一致。
			if (authorization.getScope() instanceof OrganisationScope) {
				OrganisationScope scope = (OrganisationScope) authorization
						.getScope();
				if (result != null) {
	...
	private List<OrgPermissionDTO> transformToOrgPermissionDTO(
			List<Authorization> authorizations) {
		...
		for (Authorization authorization : authorizations) {
			...
			if (authorization.getAuthority() instanceof Permission) {
				Permission permission = (Permission) authorization
						.getAuthority();
				result = new OrgPermissionDTO(permission.getId(),
						permission.getName(), permission.getIdentifier(),
	...
	public QueryChannelService getQueryChannelService() {
		if (queryChannelService == null) {
			queryChannelService = InstanceFactory.getInstance(
					QueryChannelService.class, "queryChannel_security");
		}
		return queryChannelService;
	}

	private void assembleUserJpqlAndConditionValues(
			EmployeeUserDTO queryEmployeeUserCondition, StringBuilder jpql,
			String conditionPrefix, Map<String, Object> conditionVals) {

		String andCondition = " AND " + conditionPrefix;

		if (null != queryEmployeeUserCondition.getDisabled()
				&& !"".equals(queryEmployeeUserCondition.getDisabled())) {
			jpql.append(andCondition);
			jpql.append(".disabled = :disabled");
			conditionVals.put("disabled",
					queryEmployeeUserCondition.getDisabled());
		}
		...
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getName())) {
			...
			conditionVals.put(
					...
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getName()));
		}
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getUserAccount())) {
			...
			conditionVals.put(
					"userAccount",
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getUserAccount()));
		}
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getEmail())) {
			...
			conditionVals.put(
					"email",
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getEmail()));
		}
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getTelePhone())) {
			...
			conditionVals.put(
					"telePhone",
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getTelePhone()));
		}
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getDescription())) {
			...
			conditionVals.put(
					"description",
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getDescription()));
		}
		if (!StringUtils.isBlank(queryEmployeeUserCondition.getEmployeeName())) {
			...
			conditionVals.put(
					"employeeName",
					MessageFormat.format("%{0}%",
							queryEmployeeUserCondition.getEmployeeName()));
		}
	}

	@Override
	public Page<OrgRoleDTO> pagingQueryGrantRolesByUserId(int pageIndex,
		...
		Page<Authorization> results = getQueryChannelService()//
				...
				.addParameter("actor", user).setPage(pageIndex, pageSize)//
				.pagedList();

		return new Page<OrgRoleDTO>(results.getStart(),
				results.getResultCount(), pageSize,
				transformToOrgRoleDTO(results.getData()));
	}
	 ...
	 */
	@Override
	public Page<OrgPermissionDTO> pagingQueryGrantPermissionsByUserId(
			int pageIndex, int pageSize, Long userId) {
		User user = securityAccessApplication.getUserById(userId);
		Page<Authorization> results = getQueryChannelService()//
				.createJpqlQuery(getAuthorizationsByActorJpql())//
		...
		return new Page<OrgPermissionDTO>(results.getStart(),
				results.getResultCount(), pageSize,
				transformToOrgPermissionDTO(results.getData()));

	}

	private String getAuthorizationsByActorJpql() {
		return new StringBuilder(
				"SELECT _authorization FROM Authorization _authorization JOIN _authorization.authority _authority JOIN _authorization.actor _actor WHERE _actor = :actor AND TYPE(_authority) = :authorityType")
				.toString();
	}
	 ...
	 */
	@Override
	public List<EmployeeUserDTO> queryEmployeeUsersNoRole() {
		Map<String, Object> conditionVals = new HashMap<String, Object>();
		List<EmployeeUserDTO> noRoleList = new ArrayList<EmployeeUserDTO>();
		StringBuilder jpql = new StringBuilder(
				"FROM EmployeeUser _user where 1=1");

		List<EmployeeUser> results = getQueryChannelService().createJpqlQuery(
				jpql.toString()).list();
		for (EmployeeUser user : results) {
			if (user.findAllRoles() == null || user.findAllRoles().size() <= 0)
				noRoleList.add(EmployeeUserAssembler.toEmployeeUserDTO(user));
		}
