	   ...
	   //
	    ...
	    initGridPanel: function(){
	         ...
	         var getButtons = function() {
                 return [{
                	 content: '<ks:hasSecurityResource identifier="loadBoardAdd"><button class="btn btn-primary" type="button"><span class="glyphicon glyphicon-plus"><span>添加</button></ks:hasSecurityResource>', 
                	 action: 'add'
                 },{
                	 content: '<ks:hasSecurityResource identifier="loadBoardModify"><button class="btn btn-success" type="button"><span class="glyphicon glyphicon-edit"><span>修改</button></ks:hasSecurityResource>', 
                	 action: 'modify'
                 },/* {
                	 ...
                	 content: '<ks:hasSecurityResource identifier="loadBoardChange"><button class="btn btn-primary" type="button"><span class="glyphicon glyphicon-edit"><span>状态变更</button></ks:hasSecurityResource>', 
                	 action: 'change'
                 }];
		 	 };
	         return grid.grid({
	                ...
	                pageSize : 1000,
	                isShowPages:false,
	                url:"${pageContext.request.contextPath}/LoadBoard/pageJson.koala",
	                columns: [
								 ...
								 { title: '状态', name: 'status', width: width, render: function (rowdata, name, index){
	                         	 ...
	                         	 { title: '嫁动平台', name: 'platform', width: width},
       	                         ...
       	                         { title: '可用机台', name: 'equipmentList', width: 180},
       	                      	 ...
       	                      	 { title: 'P/N(S/N)', name: 'pn', width: 120},
    	                         ...
    	                         { title: '验收单', name: 'lastFileUrl', width: width, render: function (rowdata, name, index)
	       	                     ...
	       	                     { title: '操作详情', name: 'detil', width: width, render: function (rowdata, name, index){
	       	                    	 	var h="<a href='#' onclick='loadBoardDetilView("+rowdata.id+")'>查看</a>"
	       	                    	 	return h;
	                             	 }
	                         	 }
	    ...
	    add: function(grid){
	        ...
	        var dialog = $('<div class="modal fade"><div class="modal-dialog">'
	        	...
	        	+'</div></div>');
	        $.get('<%=path%>/Kits-add.jsp').done(function(html){
	            dialog.modal({
	                keyboard:false
	            }).on({
	        ...
	        dialog.find('#save').on('click',{grid: grid}, function(e){
	              ...
	              $.post('${pageContext.request.contextPath}/LoadBoard/add.koala', dialog.find('form').serialize()).done(function(result){
	                   if(result.success ){
	                        dialog.modal('hide');
	    ...
	    modify: function(id, grid){
	        ...
	        $.get('<%=path%>/Kits-update.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	               self.initPage(dialog.find('form'));
	               $.get( '${pageContext.request.contextPath}/LoadBoard/get/' + id + '.koala').done(function(json){
	                       json = json.data
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	          		  ...
	          		  if($("input[name='acceptanceList']").val()==""||$("input[name='acceptanceList']").val()==undefined)
	        		  {
	                    ...
	                    $.post('${pageContext.request.contextPath}/LoadBoard/update.koala?id='+id, dialog.find('form').serialize()).done(function(result){
	                        if(result.success){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
	    ...
	    change: function(id, grid,data){
	        ...
	        $.get('<%=path%>/KitsOptionLog-add.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	               	var contents = [{title:'请选择', value: ''}];
	               	debugger;
	            	$.ajax({
	                ...
	                self.initPage(dialog.find('form'));
	                dialog.find("#applicableModelsID").text(data['applicableModels']);
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    ...
	                    $.post('${pageContext.request.contextPath}/LoadBoard/changeStatus.koala?id='+id, dialog.find('form').serialize()).done(function(result){
	                        if(result.success){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
...
function loadBoardDetilView(id){
	openTab('/pages/domain/loadBoardOptionLog-list.jsp', 'loadBoard操作记录', 'menuMark242');
	SocketGlobleId=id;
}
...
<!--           <label class="control-label" style="width:75px;float:left;">进场时间:&nbsp;</label>
        ...
        <div style="margin-left:15px;float:left;">
            <input name="partNo" class="form-control" type="text" style="width:180px;" id="partNoID"  />
                ...
                <label class="control-label" style="width:100px;float:left;">配件类别:&nbsp;</label>
        <div style="margin-left:15px;float:left;">
            <input name="partType" class="form-control" type="text" style="width:180px;" id="partTypeID"  />
        </div>
        <label class="control-label" style="width:100px;float:left;">适用型号:&nbsp;</label>
        <div style="margin-left:15px;float:left;">
            <input name="applicableModels" class="form-control" type="text" style="width:180px;" id="applicableModelsID"  />
        </div>
