	   ...
	   //
	    ...
	    initGridPanel: function(){
	         ...
	         var getButtons = function() {
                 return [{
                	 content: '<ks:hasSecurityResource identifier="KitsAdd"><button class="btn btn-primary" type="button"><span class="glyphicon glyphicon-plus"><span>添加</button></ks:hasSecurityResource>', 
                	 action: 'add'
                 },{
                	 content: '<ks:hasSecurityResource identifier="KitModify"><button class="btn btn-success" type="button"><span class="glyphicon glyphicon-edit"><span>修改</button></ks:hasSecurityResource>', 
                	 action: 'modify'
                 },/* {
                	 ...
                	 content: '<ks:hasSecurityResource identifier="KitsChange"><button class="btn btn-primary" type="button"><span class="glyphicon glyphicon-edit"><span>状态变更</button></ks:hasSecurityResource>', 
                	 action: 'change'
                 }];
		 	 };
	         return grid.grid({
	                ...
	                pageSize : 1000,
	                isShowPages:false,
	                url:"${pageContext.request.contextPath}/Kits/pageJson.koala",
	                columns: [
    	                         ...
    	                         { title: '验收单', name: 'lastFileUrl', width: width, render: function (rowdata, name, index)
	       	                     ...
	       	                     { title: '操作详情', name: 'detil', width: width, render: function (rowdata, name, index){
	       	                    	 	var h="<a href='#' onclick='kitsDetilView("+rowdata.id+")'>查看</a>"
	       	                    	 	return h;
	                             	 }
	                         	 }
	    ...
	    add: function(grid){
	        ...
	        dialog.find('#save').on('click',{grid: grid}, function(e){
	              ...
	              $.post('${pageContext.request.contextPath}/Kits/add.koala', dialog.find('form').serialize()).done(function(result){
	                   if(result.success ){
	                        dialog.modal('hide');
	    ...
	    modify: function(id, grid){
	        ...
	        $.get('<%=path%>/Kits-update.jsp').done(function(html){
	               ...
	               self.initPage(dialog.find('form'));
	               $.get( '${pageContext.request.contextPath}/Kits/get/' + id + '.koala').done(function(json){
	                       json = json.data
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	          		  ...
	          		  if($("input[name='acceptanceList']").val()==""||$("input[name='acceptanceList']").val()==undefined)
	        		  {
	                    ...
	                    $.post('${pageContext.request.contextPath}/Kits/update.koala?id='+id, dialog.find('form').serialize()).done(function(result){
	                        if(result.success){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
	        ...
	        $.get('<%=path%>/KitsOptionLog-add.jsp').done(function(html){
	               dialog.find('.modal-body').html(html);
	                ...
	                dialog.find('#save').on('click',{grid: grid}, function(e){
	                    ...
	                    $.post('${pageContext.request.contextPath}/Kits/changeStatus.koala?id='+id, dialog.find('form').serialize()).done(function(result){
	                        if(result.success){
	                            dialog.modal('hide');
	                            e.data.grid.data('koala.grid').refresh();
	                            e.data.grid.message({
...
function kitsDetilView(id){
	openTab('/pages/domain/KitsOptionLog-list.jsp', 'Kits操作记录', 'menuMark240');
	SocketGlobleId=id;
}
